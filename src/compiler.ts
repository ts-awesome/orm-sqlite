...
const sqlCompiler = {
  ...
  compileOrderExp(expr: {_column: string, _order: string} | {_column: string, _order: string}[]): string {
    ...
    return `${sqliteBuilder.escapeColumn(_column)} ${_order}`;
  },
  ...
  compileExp(expr: IExpr | IExpr[] | 'NULL' | '*' | any): string {
    ...
    if (_column) {
      return sqliteBuilder.escapeColumn(_column);
    }
    if (_func) {
  ...
  processColumns(columns?: (IExpr|string)[]) {
    ...
    return columns.map(column => {
      ...
      if (_alias) return `${this.compileExp(_operands)} AS ${sqliteBuilder.escapeColumn(_alias)}`;
...
function UpsertCompiler({_values, _table, _conflictExp}: IBuildableUpsertQuery): ISqlQuery[] {
  ...
  const fields = keys.map(field => sqliteBuilder.escapeColumn(field));
  const values = keys.map(field => sqlCompiler.compileExp(_values[field]));
  const updateValues = keys.map(field => `${sqliteBuilder.escapeColumn(field)} = ${sqlCompiler.compileExp(_values[field])}`);
  ...
  if(!_conflictExp) {
    ...
    sql += ` (${_conflictExp._columns.map(c => sqliteBuilder.escapeColumn(c)).join(',')})`;
...
function UpdateCompiler({_values, _where, _table, _limit}: IBuildableUpdateQuery): ISqlQuery[] {
  ...
  const values = Object
    ...
    .map(field => `${sqliteBuilder.escapeColumn(field)} = ${sqlCompiler.compileExp(_values[field])}`);

  let sql = `UPDATE ${sqliteBuilder.escapeTable(_table.tableName)} SET ${values.join(', ')}`;
